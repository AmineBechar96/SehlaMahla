...
class RejectedBookings extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.client.rejected-bookings',
        ['bookings'=>Booking::with('service')->where('user_id',auth()->user()->id)->where('status','rejected')->where(function($sub_query) {
        ...
        'number_of_bookings_rejected'=>Booking::with('service')->where('user_id',auth()->user()->id)->where('status','rejected')->count(),
        ...
        'total'=>Booking::with('service')->where('user_id',auth()->user()->id)->where('status','rejected')->where(function($sub_query) {
            $sub_query->where('title','like', '%'.trim($this->search).'%');
        })->count()]
    );
}
